...
- An asset can be provided by **either** a downloadable URL link **or** by uploading a `.mp4` file
  - Importing a URL
    - Asset in downloadable link is saved in storage
  - Uploading a file  
    - Generates a URL for storage if `Upload` option is used
    - File provided is uploaded to storage
  - Stored assets gets passed into Livepeer's decentralized transcoding network for transcoding
  - Transcoded assets gets sent to CDN for delivery
