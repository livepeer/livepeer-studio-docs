...
uploading and will be ready for playback. If `asset.status: "waiting"` is returned 
in the response, the asset is not available yet and you should make the API call again 
until `asset.status: "ready"`.
