...
The Livepeer API allows you to send video files to Livepeer and get
...
const uploadAssetURLData = await uploadAssetURL.json()
```
...
```js
const getAsset = await fetch("https://livepeer.studio/api/task/{uploadAssetURLData.asset.id}", {
    method: "GET",
    headers: {
        Authorization: `Bearer ${process.env.API_TOKEN}`,
        "Content-Type": "application/json",
