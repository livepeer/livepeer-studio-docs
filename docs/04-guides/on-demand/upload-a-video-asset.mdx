...
- If `status.phase: "waiting"` is returned in the response, the asset is not
  available:
...
## Check status of uploaded asset

To check the status of an asset uploaded with URL `/upload/url` to Livepeer or
stored to IPFS, get the `task.id` from the response of the uploaded asset.
...
> **Note:** using the `GET /api/task/$TASK_ID` request provides different
> information depending on whether the asset is _imported_ or _exported_ .
...
Use `https://livepeer.studio/api/asset/$ASSET_ID/` with the provided `asset.id`
...
> **NOTE:** If you want to track the status of your upload, you can use the
> `task` endpoint by passing the `task.id` you received during the previous
> request. To retrieve the data of your uploaded asset, you can use the `asset`
> endpoint by passing the `asset.id` as a parameter.

## List Assets

When listing assets, look for the status in the response body, it should return
`status.phase:“ready”`. If the response returns `status.phase:“waiting”`, then
wait a little longer and repeat this step until the status is ready before
moving to the exporting step.
...
it is completed. To do this, check the status field in the response.
...
This finished task will have an output with all the information about that
asset.
