...
:::caution Caveats
- Files are currently limited to 10GB in size. Any files greater than that will
  likely error out during the upload or processing steps.
- Currently, .mp4, .mov, .avi, .webm, .mkv, .wmv and .flv file types are supported. See [support matrix](https://docs.livepeer.studio/reference/support-matrix) for more information. 
:::
