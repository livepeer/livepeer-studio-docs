...
This guide provides information and instructions about minting video NFTs using
the API:
...
> **Note:** If you know how to use `tokenURI` to mint an NFT only **Step 1.**
> and use the nftMetadataUrl field as the `tokenURI` for your NFT. The URL is
...
- You can use the `@livepeer/video-nft.`
  [npm](https://www.npmjs.com/package/@livepeer/video-nft) script to have a
  local file uploaded to Livepeer, exported to IPFS, and then mint the NFT out
  of it.
...
3. Then simply run `npx @livepeer/video-nft`. The CLI will ask you for any the
   information it needs and, in the end, will give you a URL for a simple page
   for minting the NFT.
...
> **Note:** The <code>nftMetadataUrl</code> is what you need to remember for
> custom minting of the NFT.
...
testnet. If you are happy just using one of those, you can follow the link
printed at the end of the `video-nft` CLI.

A minimal UI should show where all you have to do is connect your MetaMask
wallet (currently the only supported one) and click the `Mint NFT.` button to
get your NFT setup.

![image](https://ipfs.livepeer.studio/ipfs/QmVqrWFnHi1WkJYQYNAmcVngEWMPsvZdibUFdNC4QNqXW6)

> **Note:** If you are happy with the NFT minted on the page above, you can stop
> here!
...
In this example, you will use [Alchemy](https://www.alchemy.com/) as the Eth
service provider. It is assumed you have already deployed an ERC721 contract,
which you can do so by following this
...
it in the MyNFT.json file. To use this, we'll need to parse out the contents by
adding the following lines of code to our `mint-nft.js` file:
...
- To interact with the contract, create an instance of it in the code. This
  requires the contract address which you can get from the deployment of
  [Etherscan](https://ropsten.etherscan.io/) by looking up the address you used
...
6. Next, we will use the web3 `contract` method to create our contract using the
   ABI and address. In your `mint-nft.js` file, add the following:
...
To create and send transactions to the Ethereum chain, use your public Ethereum
account address to get the account `nonce` (explained below).
...
Define a function called `mintNFT(tokenData)` and create the transaction by
doing the following:
...
2. Next, we'll need to figure out the account `nonce`; the nonce specification
   to keep track of the number of transactions sent from your address— required
   for security purposes and to prevent
   [replay attacks](https://docs.alchemyapi.io/alchemy/resources/blockchain-glossary#account-nonce).
3. To get the number of transactions sent from your address, use
...
5. "from ':
   PUBLIC_KEY`: The origin of our transaction is our public address . "to ': contractAddress`:
   The contract we wish to interact with and send the transaction
6. "nonce': nonce`: The account nonce with the number of transactions sent from
   the address
...
9. `'data': nftContract.methods.mintNFT(PUBLIC_KEY, tokenURI).encodeABI()` : The
   computation to perform in this transaction— which in this case is minting an
   NFT
...
Now that you have created the transaction, you will need to sign it to send it
off. This is where you will use your private key:

`web3.eth.sendSignedTransaction` will provide the transaction hash, which you
can use to ensure the transaction was mined and did not get dropped by the
network. You may notice in the transaction signing section some error checking
has been added to know if the transaction successfully went through.
...
Remember the `nfcMetadataUrl` from uploading your uploaded to Livepeer? Now it
is time to use it to mint the NFT.
...
pass in a new `tokenURI` describing the NFT's metadata --otherwise, you will
just end up making many identical NFTs with different IDs.
