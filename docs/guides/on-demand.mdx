...
- If you want to track the status of your upload, you can use the `asset`
  endpoint by passing the `asset.id` you received during the Request Upload request.

You can also track partial progress using the Task API using the `task.id`.
...
```
 >**NOTE** that, by default, the asset comes without the audio and video `tracks`
 >specification. You can pass `?details=true` as an additional parameter to the
 >request to get the `videoSpec` with the `tracks` included_.
