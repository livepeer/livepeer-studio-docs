...
To check the status of an asset uploaded with URL `/import` to Livepeer or exported `/export` to IPFS, get the `task.id` from the response of the uploaded asset.
