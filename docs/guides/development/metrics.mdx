...
This guide provides steps and information about using stream metrics:

- To monitor any active stream for issues that may impact the quality of your
stream 
- Understand the metrics for operating a live-streaming or user generated content platform
- View health status of your livestream 
  - Using the API for monitoring
  - No-code monitoring using the dashboard
...
| --------------------------------------- | --------------------------------------- |
| Get stream metrics with `sourceSegmensDuration`   | Getting the length of a live or recorded stream   |
| Monitor stream health | Monitor the stream health using the dashboard or the API |
...
- [Livestreaming](../livestreaming/tutorial) running
...
asset source processed by Livepeer Studio. On the parent stream object, this value
equates to the total amount of source video ingested by Livepeer Studio all time.
...
- **_Real-time —_** This indicates whether our system transcoded the stream
  faster than in real-time. In other words, if the video transcodes, latency is lower
  than the video duration. Unconventional configurations may cause an `Unhealthy` status.
- **_Multistreaming —_** This health check indicates whether all configured
  `Multistream Targets` are connected. If no targets exist, this will
  appear blank and not be considered for the global health state. If this
...
correctly, and your internet connection is good. A fluctuating or low bitrate may
suggest that your encoder is misconfigured or that the streamer internet
...
**Note:** The Stream Health API is currently in `beta` and some
    elements may change as we mature the product. Before you start building with
    it, let us know at [help@livepeer.studio](mailto:hello@livepeer.studio?subject=New%20Stream%20Health%20API%20user&body=Please%20include%20details%20about%20your%20app)
...
Stream Health API resides on a different root path `/data`. To use it,
you need to include the same API key as the one used for the regular API, check
[`API key documentation`](/guides/livestreaming/api-key) for more
...
- Events (`/data/stream/{streamId}/events`): Provides direct access to the
  ...
  subscribing via SSE to receive new events and is useful for
...
- The `healthy` top-level `condition` is a computed state based on some specific
  stream conditions. As of writing this doc, a stream is 
  `healthy` if it's both `Active`, `Transcoding` in realtime
  (`TranscodeRealTime`), and if multistream targets are configured, they are
...
- More generally, a `Condition` represents a specific state of the stream
  ...
  - The `lastTransitionsTime` and the `frequency` fields provide some insight into
...
- The `metrics` are still an experimental API and are not recommended for 
  production systems. But the `last` field is a tuple containing the
...
protocol using any SSE client. On JavaScript, we recommend
...
You can also get historical events within a time range by specifying the
