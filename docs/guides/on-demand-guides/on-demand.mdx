...
This guide walks you through the process of working with the On Demand feature:
...
- Node.js and Axios 
- An API key to call the VOD API. 
...
### Retrieve the status of that asset

### Request

Use `GET /api/asset/$ASSET_ID` 
...
Once the status indicates that it is ready, it is now available.
...
Depending on whether the asset is imported or exported using the GET /api/task/$TASK_ID provides information different information. 
