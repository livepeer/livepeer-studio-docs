...
Learn how to mint a video NFTs using the API.

- How to get started minting a video NFTs

# What you can build 
- Your own Video NFT

# What You Need to Know
- E.g., Familiarity with Livepeer Studio Concepts 
...
> **Note:**  If you know how to use `tokenURI` to mint an NFT only **Step 1.** and use the nftMetadataUrl field as the 
`tokenURI` for your NFT. The URL is compatible with both ERC-721
and ERC-1155.
...
A minimal UI should show where all you have to do is connect your
...
In this tutorial, we'll use [Alchemy](https://www.alchemy.com/) as the Eth
...
deployment of [Etherscan](https://ropsten.etherscan.io/) by looking up the
...
Now, to create and send transactions to the Ethereum chain, we'll use
...
2. Next, we'll need to figure out the account `nonce`. The nonce specification to keep track of the number of transactions sent from your address—
...
Now that we've created our transaction, we need to sign it to send it
off. Here is where we'll use our private key.

`web3.eth.sendSignedTransaction` will give us the transaction hash, which we can
use to ensure our transaction was mined and didn't get dropped by the
...
9. Call `mintNFT` and run `node scripts/mint-nft.js`
