...
The first step is understanding the `stream` object. It is the core building block of the Livepeer Studio platform.

A Livepeer Studio `stream` is a unique object with configuration data and metadata about all live stream sessions associated with it.
