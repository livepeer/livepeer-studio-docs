...
You need an API key to make requests of the Livepeer Studio API. You do not need to create an API key if you are creating and configuring live streams using only the Livepeer Studio Dashboard.

When you create a new API key, copy and secure the value displayed to you. This is the sole secret you will have to use when making requests to the Livepeer Studio API, so share and store it with caution.
...
- Create a free [Livepeer Video Service Account](https://livepeer.studio/register)
...
If you ever forget an API key you'll always be able to delete it and create a new one anyway.
